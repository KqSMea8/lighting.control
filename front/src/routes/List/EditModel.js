...
export default class EditModel extends PureComponent {
  ...
  componentDidMount = async () => {
    ...
    if (value && value.device) {
      selectOption = deviceList.find(
        item => item.deviceLocation === value.device
      );
    }
  ...
  changeDeviceSet = value => {
    ...
    const thisSelectOption = DeviceSelect.find(
      item => item.deviceIdOrGroupId === value
    );
    this.setState({
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        if (values.cityId) {
          submitData.cityId = !values.cityId["1"]
            ? values.cityId["0"]
            : values.cityId["1"];
        }
  ...
  goselectDTU = async e => {
    e.preventDefault();
    const dtu = await this.selectDTU.show(
      this.state.selectOption.deviceIdOrGroupId
    );
    this.setState({
      dtu
    });
  ...
  renderTimeMode = () => {
    ...
    return this.state.timeMode ? (
        ...
        <Col span={8}>
          <FormItem>
            {getFieldDecorator("startTime", {
              rules: [{ type: "object" }],
              initialValue: moment(
                this.props.value ? this.props.value.nodeTime : new Date(),
                "HH:mm"
              )
      ...
      <div>
        ...
        <Col span={8}>
          <FormItem>
            {getFieldDecorator("cityId", {})(<AreaSelector />)}
          </FormItem>
        </Col>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <FormItem {...formItemLayout} label="week list">
        ...
        <TimingDevice
          ref={timingdevice => {
            this.timingdevice = timingdevice;
          }}
          list={deviceList}
        />
        <DTU
          ref={selectDTU => {
            this.selectDTU = selectDTU;
          }}
        />
      </Modal>
    );

...
export default class Timing extends Component {
  ...
  dateCellRender = value => {
    ...
    if (setDate.length > 0) {
      return setDate.map(item => {
        const itemTime = moment(item);
        if (
          itemTime.date() === value.date() &&
          value.month() === itemTime.month()
        ) {
          return (
            <div
              key={`${moment()}${Math.random()}`}
              style={{
                width: "100%",
                height: "100%",
  ...
  edit = (e, id) => {
    ...
    if (this.state.operationkey === "tab1") {
      const editData = this.props.list.advancedOperation1.find(
        item => item.id === id
      );
      this.setState({
        editData,
    ...
    if (this.state.operationkey === "tab2") {
      const editData = this.props.list.advancedOperation2.find(
        item => item.id === id
      );
  ...
  render() {
    ...
    const column1 = [
      ...
      },
      {
        ...
        dataIndex: "week",
        render: text => {
          return this.state.operationkey === "tab1"
            ? text.map(item => weekStr[+item]).join(", ")
            : text.join(", ");
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <span>
              <a
                onClick={e => this.edit(e, record.id)}
                style={{ marginRight: 8 }}
              >
                编辑
              </a>

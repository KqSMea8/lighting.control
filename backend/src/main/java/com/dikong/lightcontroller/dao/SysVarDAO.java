...
public interface SysVarDAO {
    ...
    @Update({
            "update sys_var set var_value=#{varValue} where var_id=#{varId} AND proj_id=#{projId}"})
    int updateSysVar(@Param("varValue") String varValue, @Param("varId") Long varId,
    ...
    @Delete({
            "delete from sys_var where var_id=#{varId} AND sys_var_type=#{sysVarType} AND proj_id=#{projId}"})
    ...
    @Select({
            "select * from sys_var where sys_var_type=#{sysVarType} AND proj_id=#{projId} limit 1"})
    SysVar selectSequence(@Param("projId") Integer projId, @Param("sysVarType") Integer sysVarType);

    @Select({
            "select * from sys_var where sys_var_type=#{sysVarType} AND proj_id=#{projId} AND var_id=#{varId}"})
    SysVar selectByVarIdAndProjId(@Param("sysVarType") Integer sysVarType,
            @Param("projId") Integer projId, @Param("varId") Long varId);
}
